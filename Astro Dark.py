...
class Application(tk.Frame):
    ...
    def format_text(self, text):
        ...
        i = 0  # Use a separate index for the loop
        while i < len(lines):
            line = lines[i].strip()
            ...
            if len(line.split()) == 1:
                # Move the single word to the previous line if it exists
                if formatted_lines:
            ...
            if line.endswith((',', '.', ';', ':', '!', '?')):
                ...
                # If it doesn't end with punctuation, combine it with the next line if it exists
                if i + 1 < len(lines):
                    next_line = lines[i + 1].strip()
                    combined_line = f"{line} {next_line}"
                    formatted_lines.append(combined_line)
                    # Skip the next line since it's already combined
                    i += 2  # Increment by 2 to skip the next line
                else:
                    formatted_lines.append(line)
                    i += 1
